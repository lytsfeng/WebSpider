...
type myPool struct {
	...
	genEntity func() Entity //池中实体类型初始化函数
...
func (pool *myPool)Return(entity Entity) error  {
	...
	if pool.etype != reflect.TypeOf(entity){
		errMsg := fmt.Sprintf("the type of retutning entity is not %s!\n",
			pool.etype)
...
func NewPool(total uint32,entityType reflect.Type,genEntity func() Entity)  (Pool,error){
	...
	for i := 0 ; i < size ; i++{
		...
		if entityType != reflect.TypeOf(newEntity){
			errMsg := fmt.Sprintf("the type of result of function genEntity() is Not" +
					"%s!\n",entityType)
			return nil,errors.New(errMsg)
		}
		container <- newEntity
